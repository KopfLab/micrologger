...
For the particle `sddsData` events to get forwarded to the API Gateway, copy/rename the `webhook_template.json` to `webhook.json` (git-ignored) and configure the `webhook.json` with the `API-ID` and `API-KEY` from the lambda setup and run:
